...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    if ($_POST['action'] == "add_employee") {
        ...
        try {
            ...
            $stmt->bind_param("ssdi", $id, $position, $salary, $warehouse_id);
            $stmt->execute();
            $insert_message = "اطلاعات کارمند با موفقیت ثبت شد.";
            $status = "success";
        } catch (Exception $e) {
        ...
        $warehouse_id = $_POST['warehouse_id'];
        try {
            $stmt = $conn->prepare("CALL EditEmployee(?, ?, ?, ?)");
            $stmt->bind_param("ssdi", $id, $position, $salary, $warehouse_id);
            $stmt->execute();
            $alter_message = "اطلاعات کارمند با موفقیت ویرایش شد.";
            $status = "success";
        } catch (Exception $e) {
        ...
        $id = strval($_POST['id']);
        try{
            $stmt = $conn->prepare("CALL DeleteEmploye(?)");
            $stmt->bind_param("s", $id);
            $stmt->execute();
            $drop_message = "اطلاعات کارمند با موفقیت حذف شد.";
            $status = "success";
        } catch (Exception $e) {
...
// دریافت اطلاعات کارمندان برای نمایش از استور پروسیجر
$employees = [];
$sql = "CALL GetEmployees()";  // فرض می‌کنیم که یک استور پروسیجر به نام GetEmployees داریم
...
$persons = [];
$sql = "CALL GetPerson()";  // فراخوانی استور پروسیجر
$result = $conn->query($sql);
if ($result) {
    // پردازش نتایج اول
    while ($row = $result->fetch_assoc()) {
...
// دریافت اطلاعات انبارها برای نمایش از استور پروسیجر
$warehouses = [];
$sql = "CALL GetWarehouses()";  // فراخوانی استور پروسیجر برای دریافت انبارها
$result = $conn->query($sql);
if ($result) {
    // پردازش نتایج اول
    while ($row = $result->fetch_assoc()) {
...
<body>
                ...
                <tbody>
                    <?php foreach ($employees as $employee): ?>
                        <tr>
                            ...
                            <td><?php echo $employee['first_name'] . " " . $employee['last_name']. " (" . $employee['gender'] .")"; ?></td>
                            <td><?php echo htmlspecialchars($employee['position']); ?></td>
                            <td><?php echo htmlspecialchars($employee['salary']); ?></td>
                            <td><?php echo htmlspecialchars($employee['warehouse_name']); ?></td>
                            <td><?php echo htmlspecialchars($employee['capacity']); ?></td>
                            <td><?php echo htmlspecialchars($employee['location']); ?></td>
                            <td style="display: flex; align-items: center; justify-content: center;">
                                <div>
                                <button class="icon-btn" onclick="openEditModal(<?php echo htmlspecialchars(json_encode($employee)); ?>)">
    ...
    <div class="modal" id="edit-modal">
        ...
        <form method="POST" action="">
            ...
            <input type="text" name="position" id="edit-position" placeholder="سمت" required>
            <input type="number" name="salary" id="edit-salary" placeholder="حقوق" required>
    ...
    <script>
        function openEditModal(employee) {
            document.getElementById('edit-id').value = employee.EMP_ID;
            document.getElementById('edit-position').value = employee.position;
            document.getElementById('edit-salary').value = employee.salary;
            document.getElementById('edit-warehouse-id').value = employee.warehouse_foren;
